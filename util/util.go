...
func IsCheckCmd(cmd []byte) bool {
	...
	//包头包尾 7f 8f
	if cmd[0] != 0x7f || cmd[lenCmd-1] != 0x8f {
		return false
	}
	//包长度
	var cmdLen int16
	err := BytesToInt(&cmdLen, cmd[2:4])
	if lenCmd != int(cmdLen) || err != nil {
		return false
	}
	...
	if byte(check%(math.MaxUint8+1)) != cmd[cmdLen-2] {
		return false
	}
	return true
}
...
//校验包合法性 true 不合法
func IsCheckCmdByUid(cmd []byte, uid int32) bool {
